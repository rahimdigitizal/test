...
class PagesController extends Controller
{
    ...
    public function blog()
    {
        ...
        $data = [
            'posts' => Blog::where('status',1)->latest()->paginate(10),
        ];

        $all_topics = BlogCategory::latest()->take(7)->get();
        $all_tags = [];

        $recent_posts = Blog::where('status',1)->take(5)->get();
    ...
    public function blogByTag(string $tag_slug)
    {
        ...
        if ($tag) {
            ...
            $recent_posts = Blog::where('status',1)->latest()->take(5)->get();
    ...
    {
        $topic = BlogCategory::where('slug', $topic_slug)->first();

        if ($topic) {
            $posts = Blog::where('blog_category_id',$topic->id)->paginate(10);
             ...
             * Start SEO
             */
            SEOMeta::setTitle(__('seo.frontend.blog-topic', ['topic_name' => $topic->title, 'site_name' => empty($settings->setting_site_name) ? config('app.name', 'Laravel') : $settings->setting_site_name]));
            SEOMeta::setDescription('');
            SEOMeta::setCanonical(URL::current());
            SEOMeta::addKeyword($settings->setting_site_seo_home_keywords);
            /**
    ...
    {
        $post = Blog::where('slug',$blog_slug)->first();
        ...
        if ($post) {
            ...
            $sub_category = BlogSubCategory::find($post->blog_sub_category_id);

            /**
             * Start SEO
             */
            SEOMeta::setTitle($post->meta_title . ' - ' . (empty($settings->setting_site_name) ? config('app.name', 'Laravel') : $settings->setting_site_name));
