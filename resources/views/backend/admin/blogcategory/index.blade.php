...
@section('content')
    ...
    <div class="row justify-content-between">
        ...
        <div class="col-3 text-right">
            <a href="{{ url('admin/category/blog/create') }}" class="btn btn-info btn-icon-split">
                                    ...
                                    <tbody>
                                        @foreach ($categories as $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->slug }}</td>
                                                <td class="d-flex">
                                                    <a href="{{ url('admin/category/blog/'.$item->id.'/edit') }}" class="btn btn-outline-info btn-sm">Edit</a>
                                                    <form action="{{ url('admin/category/blog/'.$item->id) }}" method="post" class="ml-2">
                                                        @csrf
                                                        @method('DELETE')
